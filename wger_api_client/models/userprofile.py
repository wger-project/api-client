...
class Userprofile:
    ...
    Attributes:
        ...
        gym (int | None):
        is_temporary (bool):
        last_workout_notification (datetime.date | None):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        last_workout_notification: None | str
        if isinstance(self.last_workout_notification, datetime.date):
            last_workout_notification = self.last_workout_notification.isoformat()
        else:
            last_workout_notification = self.last_workout_notification
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        ...
        is_trustworthy = d.pop("is_trustworthy")

        date_joined = datetime.datetime.fromisoformat(d.pop("date_joined"))

        def _parse_gym(data: object) -> int | None:
            if data is None:
                return data
            return cast(int | None, data)

        gym = _parse_gym(d.pop("gym"))

        is_temporary = d.pop("is_temporary")

        def _parse_last_workout_notification(data: object) -> datetime.date | None:
            if data is None:
                return data
            try:
                if not isinstance(data, str):
                    raise TypeError()
                last_workout_notification_type_0 = datetime.date.fromisoformat(data)

                return last_workout_notification_type_0
            except (TypeError, ValueError, AttributeError, KeyError):
                pass
            return cast(datetime.date | None, data)

        last_workout_notification = _parse_last_workout_notification(
            d.pop("last_workout_notification")
        )
